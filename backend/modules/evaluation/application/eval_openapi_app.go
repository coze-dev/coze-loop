...
func (e *EvalOpenAPIApplication) RunEvaluatorOApi(ctx context.Context, req *openapi.RunEvaluatorOApiRequest) (r *openapi.RunEvaluatorOApiResponse, err error) {
	...
	record, err := e.evaluatorService.RunEvaluator(ctx, &entity.RunEvaluatorRequest{
		...
		EvaluatorRunConf:   evaluator_convertor.OpenAPIEvaluatorRunConfigDTO2DO(req.EvaluatorRunConf),
		Ext:                req.Ext,
	})
	if err != nil {
		return nil, err
...
func (e *EvalOpenAPIApplication) ListExptTemplatesOApi(ctx context.Context, req *openapi.ListExptTemplatesOApiRequest) (r *openapi.ListExptTemplatesOApiResponse, err error) {
	...
	if req.FilterOption != nil && req.FilterOption.Filters != nil {
		filter = &entity.ExptTemplateListFilter{
			Includes: &entity.ExptTemplateFilterFields{},
		}
		for _, cond := range req.FilterOption.Filters.FilterConditions {
			if cond == nil {
				continue
			}
			switch cond.GetField() {
			case "name":
				filter.FuzzyName = cond.GetValue()
			case "expt_type":
				t := experiment.ExperimentType(cond.GetValue())
				filter.Includes.ExptType = append(filter.Includes.ExptType, int64(experiment_convertor.OpenAPIExptTypeDTO2DO(&t)))
			}
		}
	}
