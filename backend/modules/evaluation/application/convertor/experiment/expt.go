...
func ToExptDTO(experiment *entity.Experiment) *domain_expt.Experiment {
	...
	if experiment.EnableScoreWeight || len(evalWeights) > 0 {
		enable := experiment.EnableScoreWeight || len(evalWeights) > 0
		res.EnableWeightedScore = gptr.Of(enable)
		res.ScoreWeightConfig = &domain_expt.ExptScoreWeight{
			EnableWeightedScore:   gptr.Of(enable),
	...
	if experiment.ExptTemplateMeta != nil {
		res.ExptTemplateMeta = &domain_expt.ExptTemplateMeta{
			...
			CreatorBy:   gptr.Of(experiment.ExptTemplateMeta.CreatorBy),
