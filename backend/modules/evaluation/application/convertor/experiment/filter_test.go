...
func TestExptFilterConvertor_Convert_NilOption(t *testing.T) {
	...
	// filter_option 为 nil 时仍返回非 nil filter; EvalSetSourceTypes 留空, 由 DAO 层默认排除 MultiSetConfig(2)。
...
// 前端把「评测集 → item_key」按 EvalSetColumn(42) + field_key=item_key 提交，
// 但 item_key 是 dataset_item_snapshot 的独立列、不在评测集 schema mapping 里。
// 必须分流到 ColumnFilters，否则落进 StringMapFilters 会 mapping miss → 条件被静默丢弃 → 退化全量扫描。
func TestConvertExptTurnResultFilterAccelerator_ItemKeyGoesToColumnFilters(t *testing.T) {
	t.Run("item_key 走独立列条件", func(t *testing.T) {
		got, err := ConvertExptTurnResultFilterAccelerator(&domain_expt.ExperimentFilter{
			Filters: &domain_expt.Filters{
				LogicOp: domain_expt.FilterLogicOpPtr(domain_expt.FilterLogicOp_And),
				FilterConditions: []*domain_expt.FilterCondition{
					{
						Field:    &domain_expt.FilterField{FieldType: domain_expt.FieldType_EvalSetColumn, FieldKey: gptr.Of("item_key")},
						Operator: domain_expt.FilterOperatorType_Like,
						Value:    "feature_vuelidate_vuelidate_pr_1066",
					},
				},
			},
		})
		require.NoError(t, err)
		require.NotNil(t, got.ItemSnapshotCond)
		require.Len(t, got.ItemSnapshotCond.ColumnFilters, 1)
		assert.Equal(t, "item_key", got.ItemSnapshotCond.ColumnFilters[0].Key)
		assert.Equal(t, "LIKE", got.ItemSnapshotCond.ColumnFilters[0].Op)
		// 关键：不能同时落进 StringMapFilters，否则仍会去查 mapping
		assert.Empty(t, got.ItemSnapshotCond.StringMapFilters)
	})

	t.Run("普通评测集 schema 字段仍走 string_map（不受影响）", func(t *testing.T) {
		got, err := ConvertExptTurnResultFilterAccelerator(&domain_expt.ExperimentFilter{
			Filters: &domain_expt.Filters{
				LogicOp: domain_expt.FilterLogicOpPtr(domain_expt.FilterLogicOp_And),
				FilterConditions: []*domain_expt.FilterCondition{
					{
						Field:    &domain_expt.FilterField{FieldType: domain_expt.FieldType_EvalSetColumn, FieldKey: gptr.Of("my_schema_field")},
						Operator: domain_expt.FilterOperatorType_Equal,
						Value:    "v",
					},
				},
		...
		require.NoError(t, err)
		require.Len(t, got.ItemSnapshotCond.StringMapFilters, 1)
		assert.Equal(t, "my_schema_field", got.ItemSnapshotCond.StringMapFilters[0].Key)
		assert.Empty(t, got.ItemSnapshotCond.ColumnFilters)
	})

	t.Run("keyword_search 里的 item_key 同样走独立列", func(t *testing.T) {
		got, err := ConvertExptTurnResultFilterAccelerator(&domain_expt.ExperimentFilter{
			Filters: &domain_expt.Filters{LogicOp: domain_expt.FilterLogicOpPtr(domain_expt.FilterLogicOp_And)},
			KeywordSearch: &domain_expt.KeywordSearch{
				Keyword: gptr.Of("pr_1066"),
				FilterFields: []*domain_expt.FilterField{
					{FieldType: domain_expt.FieldType_EvalSetColumn, FieldKey: gptr.Of("item_key")},
				},
			},
		})
		require.NoError(t, err)
		require.NotNil(t, got.KeywordSearch)
		require.Len(t, got.KeywordSearch.ItemSnapshotFilter.ColumnFilters, 1)
		assert.Equal(t, "item_key", got.KeywordSearch.ItemSnapshotFilter.ColumnFilters[0].Key)
		assert.Empty(t, got.KeywordSearch.ItemSnapshotFilter.StringMapFilters)
	})
}
