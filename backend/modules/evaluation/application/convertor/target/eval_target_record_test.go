...
func TestToInvokeOutputDataDO_StepsRoundTrip(t *testing.T) {
	successStatus := spi.InvokeEvalTargetStatus_SUCCESS
	req := &openapi.ReportEvalTargetInvokeResultRequest{
		Status: &successStatus,
		Output: &spi.InvokeEvalTargetOutput{
			Steps: []*spi.EvalTargetStep{
				{
					StepName:    gptr.Of("plan"),
					TurnIndex:   gptr.Of(int32(0)),
					StepIndex:   gptr.Of(int32(1)),
					StartTimeMs: gptr.Of(int64(1000)),
					EndTimeMs:   gptr.Of(int64(1500)),
					DurationMs:  gptr.Of(int64(500)),
				...
				nil, // nil 项应被跳过
				{
					...
					ErrorCode:    gptr.Of(int32(601200702)),
					ErrorMessage: gptr.Of("crashed"),
	...
	assert.Equal(t, "plan", out.Steps[0].StepName)
	assert.Equal(t, int32(1), out.Steps[0].StepIndex)
	assert.Equal(t, int64(500), out.Steps[0].DurationMS)
	assert.True(t, out.Steps[0].Success)
	assert.Equal(t, "v", out.Steps[0].Ext["k"])
	assert.Equal(t, "act", out.Steps[1].StepName)
	assert.Equal(t, int32(601200702), out.Steps[1].ErrorCode)
	assert.Equal(t, "crashed", out.Steps[1].ErrorMessage)
	assert.False(t, out.Steps[1].Success)
}

func TestToInvokeStepsDO_EmptyReturnsNil(t *testing.T) {
	assert.Nil(t, ToInvokeStepsDO(nil))
	assert.Nil(t, ToInvokeStepsDO([]*spi.EvalTargetStep{}))
}
