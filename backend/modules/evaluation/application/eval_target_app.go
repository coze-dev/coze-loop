...
func (e EvalTargetApplicationImpl) listSharedSourceEvalTargets(ctx context.Context, request *eval_target.ListSourceEvalTargetsRequest) (*eval_target.ListSourceEvalTargetsResponse, error) {
	...
	sourceSpaces, accessCtxByResource, err := e.sharedSourceSpaces(ctx, request.WorkspaceID, request.SharedOption)
	if err != nil {
		return nil, err
	}
	dtos := make([]*eval_target_dto.EvalTarget, 0)
	for _, src := range sourceSpaces {
		...
		for _, do := range res {
			...
			resourceID, parseErr := strconv.ParseInt(do.SourceTargetID, 10, 64)
			if parseErr != nil {
				continue
			}
			accessCtx, ok := accessCtxByResource[sharedResourceKey(src, resourceID)]
			if !ok {
				continue
			}
...
func (e EvalTargetApplicationImpl) sharedSourceSpaces(ctx context.Context, callerSpaceID int64, sharedOption *common.SharedResourceOption) ([]int64, map[string]*entity.ResourceAccessContext, error) {
