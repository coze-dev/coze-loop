...
func TestHTTPFaaSIntegration(t *testing.T) {
	...
	t.Run("JavaScript代码执行", func(t *testing.T) {
		...
		result, err := jsAdapter.RunCode(ctx, code, "javascript", 10000)
		require.NoError(t, err)
...
print(f"Result: {result}")
		...
		result, err := pythonAdapter.RunCode(ctx, code, "python", 10000)
		require.NoError(t, err)
		assert.NotNil(t, result)
		assert.NotNil(t, result.Output)
	...
	t.Run("超时处理", func(t *testing.T) {
		...
		start := time.Now()
		result, err := jsAdapter.RunCode(ctx, code, "javascript", 1000) // 1秒超时
	...
	t.Run("并发执行", func(t *testing.T) {
		...
		for i := 0; i < concurrency; i++ {
			go func(index int) {
				...
				_, err := jsAdapter.RunCode(ctx, code, "javascript", 5000)
				results <- err
			}(i)
		}
