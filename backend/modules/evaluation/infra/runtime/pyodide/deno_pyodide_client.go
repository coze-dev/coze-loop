...
func NewDenoPyodideRuntime(config *entity.SandboxConfig, logger *logrus.Logger) (*DenoPyodideRuntime, error) {
	...
	possiblePaths := []string{
		filepath.Join(wd, "infra", "runtime", "pyodide", "pyodide_runner.ts"),                                                    // 当前目录下的相对路径
		filepath.Join(wd, "backend", "modules", "evaluation", "infra", "runtime", "pyodide", "pyodide_runner.ts"),   // 完整路径
		filepath.Join(wd, "modules", "evaluation", "infra", "runtime", "pyodide", "pyodide_runner.ts"),              // 相对路径
		filepath.Join(wd, "runtime", "pyodide", "pyodide_runner.ts"),                                        // runtime目录
		"./infra/runtime/pyodide/pyodide_runner.ts",                                                                           // 相对于当前工作目录
	}
...
// RunCode 执行Python代码
func (dr *DenoPyodideRuntime) RunCode(ctx context.Context, req *entity.SandboxExecutionRequest) (*entity.SandboxExecutionResult, error) {
	startTime := time.Now()

	// 验证请求
	if req == nil {
		return &entity.SandboxExecutionResult{
			Error:    "执行请求不能为空",
			ExitCode: 1,
			Success:  false,
...
func (dr *DenoPyodideRuntime) executeWithPipeComm(ctx context.Context, req *entity.SandboxExecutionRequest, startTime time.Time) (*entity.SandboxExecutionResult, error) {
	// 构建配置
...
func (dr *DenoPyodideRuntime) executeAttempt(
	...
	requestData []byte, 
	startTime time.Time,
) (*entity.SandboxExecutionResult, error) {
	// 创建管道
	pr, pw, err := os.Pipe()
	if err != nil {
...
func (dr *DenoPyodideRuntime) createErrorResult(message string, err error, startTime time.Time) *entity.SandboxExecutionResult {
	errorMsg := message
	if err != nil {
...
func (dr *DenoPyodideRuntime) buildConfig(req *entity.SandboxExecutionRequest) *SandboxConfig {
...
func (dr *DenoPyodideRuntime) convertExecutionResult(result *ExecutionResult, duration time.Duration) *entity.SandboxExecutionResult {
	entityResult := &entity.SandboxExecutionResult{
