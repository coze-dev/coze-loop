...
func (ExptConverter) PO2DO(expt *model.Experiment, refs []*model.ExptEvaluatorRef) (*entity.Experiment, error) {
	...
	if len(gptr.Indirect(expt.NotificationConf)) > 0 {
		...
		if err := json.Unmarshal(gptr.Indirect(expt.NotificationConf), notificationConf); err != nil {
			return nil, errorx.Wrapf(err, "ExptNotificationConf json unmarshal fail, expt_id: %v, raw: %v", expt.ID, conv.UnsafeBytesToString(gptr.Indirect(expt.NotificationConf)))
		}
	}
