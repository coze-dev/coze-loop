...
func TestExptTemplateDAOImpl_toConditions_CronActivateAndValidation(t *testing.T) {
	...
	t.Run("malicious order by field is ignored and default order applies", func(t *testing.T) {
		inject := "id,(select updatexml(1,concat(0x7e,database(),0x7e),1) from(select 1)x)"
		conds, ok := dao.toConditions(nil, []*entity.OrderBy{{Field: &inject}})
		assert.True(t, ok)
		// 非法列名被丢弃，仅保留默认 created_at desc
		assert.Len(t, conds, 1)
	})
...
func TestExptTemplateDAOImpl_List_SQLShapes(t *testing.T) {
	...
	t.Run("join query uses group explicit order and paging", func(t *testing.T) {
		...
		findSQL := "SELECT `expt_template`\\.`id`,`expt_template`\\.`space_id`,`expt_template`\\.`name`,`expt_template`\\.`description`,`expt_template`\\.`eval_set_id`,`expt_template`\\.`eval_set_version_id`,`exptemplate`\\.`target_id`,`expt_template`\\.`target_type`,`expt_template`\\.`target_version_id`,`expt_template`\\.`expt_type`,`expt_template`\\.`cron_activate`,`exptemplate`\\.`template_conf`,`exptemplate`\\.`expt_info`,`exptemplate`\\.`created_by`,`expt_template`\\.`updated_by`,`expt_template`\\.`created_at`,`expt_template`\\.`updated_at`,`expt_template`\\.`deleted_at` FROM `expt_template` INNER JOIN expt_template_evaluator_ref ON expt_template.id = exptemplate_evaluator_ref.exptemplate_id WHERE exptemplate.space_id = \\? AND exptemplate.deleted_at IS NULL AND exptemplate_evaluator_ref.evaluator_id IN \\(\\?\\) AND `exptemplate`\\.`deleted_at` IS NULL GROUP BY `exptemplate`\\.`id` ORDER BY exptemplate.created_at asc LIMIT \\? OFFSET \\?"
