...
func (m *MockEvaluatorTagDAO) GetTagsBySourceIDAndType(ctx context.Context, sourceID int64, tagType int32, opts ...db.Option) ([]*model.EvaluatorTag, error) {
	args := m.Called(ctx, sourceID, tagType, opts)
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedError {
				...
				for i, template := range result.Templates {
					...
					assert.Equal(t, expected.Hot, template.Hot)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expected == nil {
				...
				assert.Equal(t, tt.expected.Hot, result.Hot)
