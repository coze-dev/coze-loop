...
func (e *exptEventPublisher) PublishWebhookDeliveryEvent(ctx context.Context, events []*entity.WebhookDeliveryEvent, duration *time.Duration) error {
	if len(events) == 0 {
		return nil
	}
	return e.batchSend(ctx, rocket.WebhookDeliveryEventRMQKey, lo.ToAnySlice(events), duration)
}
