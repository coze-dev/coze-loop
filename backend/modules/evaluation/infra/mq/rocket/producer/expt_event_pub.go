...
func newExptEventPublisher(ctx context.Context, cfgFactory conf.IConfigLoaderFactory, mqFactory mq.IFactory) (events.ExptEventPublisher, error) {
	...
	for _, key := range []string{
		...
		rocket.WebhookRetryEventRMQKey,
	} {
		p := &producer{}
...
func (e *exptEventPublisher) PublishWebhookRetryEvent(ctx context.Context, event *entity.WebhookRetryEvent, duration *time.Duration) error {
	return e.batchSend(ctx, rocket.WebhookRetryEventRMQKey, []any{event}, duration)
}
