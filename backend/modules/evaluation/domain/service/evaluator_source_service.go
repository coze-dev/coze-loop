...
type EvaluatorSourceService interface {
	...
	// 返回值: skip 表示是否劫持, output 为劫持时的输出数据(含劫持原因等，可为nil)。
	ShouldIntercept(ctx context.Context, evaluator *entity.Evaluator, input *entity.EvaluatorInputData) (output *entity.EvaluatorOutputData, skip bool)
