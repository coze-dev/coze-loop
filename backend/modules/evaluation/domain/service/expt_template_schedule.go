...
// scheduleCallbackMethod ByteScheduler 触发时回调的 RPC 方法名（ExperimentService.SubmitExptFromTemplate）
const scheduleCallbackMethod = "SubmitExptFromTemplate"

// scheduleBizKeyPrefix BizKey 前缀，便于在调度平台侧按业务区分定时任务来源
const scheduleBizKeyPrefix = "expt_template_schedule"

// schedulerCallbackPayload 调用 SubmitExptFromTemplate 的 JSON Body（与方法 req 字段一致，使用 JSON number 以便后端反序列化为 int64）。
...
// buildSchedulerCallbackPayload 序列化 ByteScheduler 触发时的回调请求体。
