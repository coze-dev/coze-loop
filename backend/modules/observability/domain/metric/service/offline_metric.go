...
func (m *MetricsService) buildExtraFilter(req *QueryMetricsReq, mDef entity.IMetricDefinition) []*loop_span.FilterField {
	...
	notDrillDownKeys := make([]string, 0)
	for _, key := range m.pMetrics.PlatformMetricDefs[req.PlatformType].DrillDownObjects {
		if !requestFieldName[key] {
			notDrillDownKeys = append(notDrillDownKeys, key)
		}
	}
	for _, key := range m.metricDrillDown[mDef.Name()] {
		if !requestFieldName[key] {
			notDrillDownKeys = append(notDrillDownKeys, key)
		}
	}
	ret := make([]*loop_span.FilterField, 0)
	for _, key := range notDrillDownKeys {
