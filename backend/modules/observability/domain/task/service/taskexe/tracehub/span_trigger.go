...
func (h *TraceHubServiceImpl) SpanTrigger(ctx context.Context, rawSpan *entity.RawSpan) error {
	...
	if !gslice.Contains(spaceIDs, span.WorkspaceID) && !gslice.Contains(botIDs, span.TagsString["bot_id"]) {
		logs.CtxInfo(ctx, "no space or bot found for span,log_suffix=%s,space_id=%s,bot_id=%s,space list: %v,bot list: %v", span.WorkspaceID, span.TagsString["bot_id"], logSuffix, spaceIDs, botIDs)
		return nil
	}
