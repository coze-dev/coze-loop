...
func TestAutoEvaluateProcessor_Invoke_TerminateOnExperimentFailedCode(t *testing.T) {
	t.Parallel()
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	// Construct task and run
	workspaceID := int64(12345)
	taskID := int64(1001)
	runID := int64(2002)

	// Prepare evaluation set schema JSON
	...
	// Configure mapping so buildItems produces non-empty turns
	...
	// Expectations: counts inc/reads, then dec on error
	// 注意：TTL 依赖当前时间计算，存在毫秒级波动，放宽期望为任意 int64
	...
	// Expectations: terminate task and run
	repoMock.EXPECT().UpdateTask(gomock.Any(), gomock.AssignableToTypeOf(&taskentity.ObservabilityTask{})).Return(nil).Times(1)
	repoMock.EXPECT().RemoveNonFinalTask(gomock.Any(), strconv.FormatInt(workspaceID, 10), taskID).Return(nil).Times(1)
	repoMock.EXPECT().UpdateTaskRun(gomock.Any(), gomock.AssignableToTypeOf(&taskentity.TaskRun{})).Return(nil).Times(1)

	// evaluation adapter returns business error code 601204012
	...
	// Assert task disabled and run marked done
	assert.Equal(t, taskentity.TaskStatusFailed, taskObj.TaskStatus)
	assert.Equal(t, taskentity.TaskRunStatusDone, taskRun.RunStatus)
}
