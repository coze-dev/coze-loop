...
func TestTraceCkRepoImpl_ListPreSpans(t *testing.T) {
	type fields struct {
		spansDao    ck.ISpansDao
		traceConfig config.ITraceConfig
	}
	...
	tests := []struct {
		...
		{
			name: "list pre spans successfully",
			fieldsGetter: func(ctrl *gomock.Controller) fields {
				spansDaoMock := ckmock.NewMockISpansDao(ctrl)
				spansDaoMock.EXPECT().Get(gomock.Any(), gomock.Any()).Return([]*model.ObservabilitySpan{
					{
						...
						Method:        ptr.Of("method1"),
						CallType:      ptr.Of("call1"),
						ObjectStorage: ptr.Of("storage1"),
					},
					{
						TraceID: "trace1",
						SpanID:  "span1",
		...
		{
			name: "list pre spans with filters and select columns",
			fieldsGetter: func(ctrl *gomock.Controller) fields {
				spansDaoMock := ckmock.NewMockISpansDao(ctrl)
				spansDaoMock.EXPECT().Get(gomock.Any(), gomock.Any()).Return([]*model.ObservabilitySpan{
					{
			...
			args: args{
				...
				req: &repo.ListPreSpansParam{
					...
					Limit:         5,
					Filters:       &loop_span.FilterFields{},
					SelectColumns: []string{"trace_id", "span_id"},
					OmitColumns:   []string{"tags"},
				},
			},
			want: loop_span.SpanList{
				{
		...
		{
			name: "list pre spans failed due to config error",
			fieldsGetter: func(ctrl *gomock.Controller) fields {
				traceConfigMock := confmocks.NewMockITraceConfig(ctrl)
				traceConfigMock.EXPECT().GetTenantConfig(gomock.Any()).Return(nil, assert.AnError)
				return fields{
		...
		{
			name: "list pre spans failed due to dao error",
			fieldsGetter: func(ctrl *gomock.Controller) fields {
				spansDaoMock := ckmock.NewMockISpansDao(ctrl)
		...
		{
			name: "list pre spans with empty result",
			fieldsGetter: func(ctrl *gomock.Controller) fields {
				spansDaoMock := ckmock.NewMockISpansDao(ctrl)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := r.ListPreSpans(tt.args.ctx, tt.args.req)
			assert.Equal(t, tt.wantErr, err != nil)
			assert.Equal(t, tt.want, got)
		})
	}
