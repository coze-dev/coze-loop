...
func TestMetricApplication_GetMetrics(t *testing.T) {
	...
	t.Run("success without compare", func(t *testing.T) {
		...
		defer ctrl.Finish()
		svc := metricservicemock.NewMockIMetricsService(ctrl)
		...
		svc.EXPECT().QueryMetrics(gomock.Any(), gomock.Any()).DoAndReturn(func(_ context.Context, req *service.QueryMetricsReq) (*service.QueryMetricsResp, error) {
			mu.Lock()
			captured = append(captured, req)
			mu.Unlock()
			assert.Equal(t, []string{"metric_a"}, req.MetricsNames)
		...
		if assert.Len(t, captured, 1) {
			assert.Equal(t, req.GetStartTime(), captured[0].StartTime)
			assert.Equal(t, req.GetEndTime(), captured[0].EndTime)
