...
func SchemaKeyFromString(s string) (SchemaKey, error) {
	...
	case "MessageList":
		return SchemaKey_MessageList, nil
	}
...
type Dataset struct {
	...
	// 数据集业务唯一键，创建后不可变
	DatasetKey *string `thrift:"dataset_key,23,optional" frugal:"23,optional,string" form:"dataset_key" json:"dataset_key,omitempty" query:"dataset_key"`
	/* 通用信息 */
...
func (p *Dataset) writeField23(oprot thrift.TProtocol) (err error) {
	if p.IsSetDatasetKey() {
		if err = oprot.WriteFieldBegin("dataset_key", thrift.STRING, 23); err != nil {
			goto WriteFieldBeginError
		}
...
func (p *Dataset) DeepEqual(ano *Dataset) bool {
	...
	if !p.Field23DeepEqual(ano.DatasetKey) {
		return false
	}
