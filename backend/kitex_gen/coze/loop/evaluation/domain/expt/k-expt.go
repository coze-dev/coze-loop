...
func (p *Experiment) FastReadField72(buf []byte) (int, error) {
	offset := 0
...
func (p *Experiment) FastWriteNocopy(buf []byte, w thrift.NocopyWriter) int {
	...
	if p != nil {
		...
		offset += p.fastWriteField72(buf[offset:], w)
...
func (p *Experiment) fastWriteField72(buf []byte, w thrift.NocopyWriter) int {
	offset := 0
	if p.IsSetNotificationConf() {
		offset += thrift.Binary.WriteFieldBegin(buf[offset:], thrift.STRUCT, 72)
...
func (p *Experiment) field72Length() int {
	l := 0
	if p.IsSetNotificationConf() {
		l += thrift.Binary.FieldBeginLength()
