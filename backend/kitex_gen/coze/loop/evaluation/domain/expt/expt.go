...
type ItemSystemInfo struct {
	...
	// 该 item 在最新一轮运行内评测对象的调用次数（含系统自动重试）；来自 eval_target_record 按 (run,item,turn) 聚合
...
type RetryRecord struct {
	...
	// 是否为最终采用的那次（= expt_turn_result.target_result_id 指向）
	...
	// 回放日志 agent_replay_<id>.tar.gz（用户最关心）
	...
	// agent 沙箱日志 agent_sandbox_<id>.tar.gz
