...
struct CreateExperimentTemplateRequest {
    ...
    // 评测对象与评测集配置
    10: optional i64 eval_set_id (api.body='eval_set_id', api.js_conv='true', go.tag='json:"eval_set_id"')
    11: optional i64 eval_set_version_id (api.body='eval_set_version_id', api.js_conv='true', go.tag='json:"eval_set_version_id"')
    12: optional i64 target_id (api.body='target_id', api.js_conv='true', go.tag='json:"target_id"')
    13: optional i64 target_version_id (api.body='target_version_id', api.js_conv='true', go.tag='json:"target_version_id"')
    ...
    31: optional bool enable_weighted_score (api.body = 'enable_weighted_score', go.tag='json:"enable_weighted_score"')
    32: optional map<i64, double> evaluator_score_weights (api.body = 'evaluator_score_weights', go.tag='json:"evaluator_score_weights"')

    // 默认实验运行配置
    40: optional i32 default_item_concur_num (api.body = 'default_item_concur_num')
    41: optional i32 default_evaluators_concur_num (api.body = 'default_evaluators_concur_num')
    42: optional string schedule_cron (api.body = 'schedule_cron')

    50: optional expt.ExptType expt_type (api.body = 'expt_type')
...
struct UpdateExperimentTemplateRequest {
    ...
    // 默认版本与配置（注意：eval_set_id / target_id 不允许修改，仅允许调整版本与配置）
    10: optional i64 eval_set_version_id (api.body='eval_set_version_id', api.js_conv='true', go.tag='json:"eval_set_version_id"')
    11: optional i64 target_version_id (api.body='target_version_id', api.js_conv='true', go.tag='json:"target_version_id"')
...
service ExperimentService {
    ...
    // 实验模板
    CreateExperimentTemplateResponse CreateExperimentTemplate(1: CreateExperimentTemplateRequest req) (api.post = '/api/evaluation/v1/experiment_templates/create')
    GetExperimentTemplateResponse GetExperimentTemplate(1: GetExperimentTemplateRequest req) (api.post = '/api/evaluation/v1/experiment_templates/:template_id')
