...
struct Filter {
  1: optional QueryRelation queryAndOr,
  2: required list<FilterField> filterFields
}
...
struct FieldMeta {
    // 字段类型
    1: required FieldType fieldType (agw.key = "field_type")
    // 当前字段支持的操作类型
    2: required list<QueryType> queryTypes (agw.key = "query_types")
