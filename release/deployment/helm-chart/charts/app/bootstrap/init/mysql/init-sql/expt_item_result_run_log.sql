...
(
    ...
    `retry_times`     int unsigned    NOT NULL DEFAULT '0' COMMENT '本轮实验运行中该 item 已被系统自动重试的次数; 0=未重试; 仅内部调度降权用, 不透出',
    PRIMARY KEY (`id`),
    ...
    KEY `idx_expt_run_retry_pick` (`space_id`, `expt_id`, `expt_run_id`, `status`, `retry_times`, `id`)
) ENGINE = InnoDB
