...
/**
 * coze-loop-faas-unified 统一FaaS服务器
 * 通过配置参数控制基础模式和增强模式
...
async function main() {
  ...
  console.log(`启动统一FaaS服务器，模式: ${config.mode}，端口: ${config.port}...`);
  ...
  console.log(`统一FaaS服务器启动成功: http://0.0.0.0:${config.port}`);
  console.log("可用端点:");
...
if (import.meta.main) {
  try {
    ...
    console.error("启动统一FaaS服务器失败:", error);
    Deno.exit(1);
  }
}